(function() {
    ...
    18: [ function(D, h, z) {
        ...
        const F = D("zO"), l = j(D("Fc")), Z = j(D("yI")), A = {};
        class q {
            constructor() {
                this.storage = new l.default;
            }
            run() {
                this.onInstall(), this.initOnActionClick(), this.initOnMessageListener(), this.initOnUpdated(),
                this.initOnTabRemove(), chrome.tabs.query({}, (D => D.forEach((D => this.switchAction(D)))));
            }
            checkTabIsValid(D) {
                const h = D.url;
                if (!h) return false;
                if (h.startsWith("chrome:") || h.startsWith("ftp:") || h.startsWith("file:")) return false;
                return true;
            }
            pageEvent(D) {
                const h = chrome.runtime.getURL("");
                if (!D.id) return;
                if (D.url && D.url.startsWith(h)) {
                    const h = {
                        action: F.MessageAction.CLOSE_READER
                    };
                    chrome.tabs.sendMessage(D.id, h);
                    const z = String(D.favIconUrl).replace("chrome://favicon/", "");
            ...
            enableAction(D) {
                chrome.action.enable(D);
            }
            switchAction(D) {
                if (!D.id || !D.url) return;
                if (!this.checkTabIsValid(D) && D.id) this.disableAction(D.id); else this.enableAction(D.id);
            }
            initOnUpdated() {
            ...
            onInstall() {
                chrome.runtime.onInstalled.addListener((D => {
                    if (this.initContextMenu(), D.reason === chrome.runtime.OnInstalledReason.INSTALL) l.default.init();
                }));
            }
            initOnTabRemove() {
                chrome.tabs.onRemoved.addListener((D => delete A[D]));
            }
            sendNotification(D) {
                const h = chrome.i18n.getMessage("extName") || "Reader View";
            ...
            async openReader(D, h) {
                ...
                A[j] = {
                    ...
                    [z]: 1
                }), chrome.tabs.update(j, {
                    url: chrome.runtime.getURL("html/reader.html?id=" + j)
                });
            }
            initOnMessageListener() {
                chrome.runtime.onMessage.addListener(((D, h, z) => {
                    ...
                    const l = D.action;
                    switch (l) {
                      case F.MessageAction.OPEN_READER:
                        if (h.tab) this.openReader(D.data, h.tab);
                        break;

                      case F.MessageAction.OPEN_ON_TAB:
                        if (h.tab) this.pageEvent(h.tab);
                        break;

                      case F.MessageAction.GET_ARTICLE:
                        if ((j = h.tab) === null || j === void 0 ? void 0 : j.id) z(A[h.tab.id]); else z(null);
                        break;

                      case F.MessageAction.SEND_NOTIFICATION:
                        this.sendNotification(D.data);
                        break;

                      case F.MessageAction.OPEN_URL:
                        chrome.tabs.create({
                            url: D.data
                        });
                        break;
        ...
        const Q = new q;
        Q.run(), (0, Z.default)("G-N6J6EP2CD8", "wDVDXBNgR3-uZkZ2ePIjdw");
    }, {
        yI: 1,
        zO: 19,
        Fc: 21
